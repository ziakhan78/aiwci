...
public partial class Admin_ViewExecutiveCommittee : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (Session["user"] != null)
        {
            if (!IsPostBack)
            {
                lblHeading.Text = "Executive Committee List";
                BindDistrictNo();
            }
        }
        else
    ...
    private void GetExecutiveCommittee(string districtNo)
    {
        PresidentBll obj = new PresidentBll();
        DataTable dt = new DataTable();
        obj.DistrictNo = districtNo;        
        dt = obj.GetExecutiveCommittee();
        if (dt.Rows.Count > 0)
        {
    ...
    {        
        ...
        GetExecutiveCommittee(districtNo);
    }
